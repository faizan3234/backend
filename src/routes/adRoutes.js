  ...
  for (const venueId of venueIds) {
    for (let day = startDate; day <= endDate; day = addCalendarDays(day, 1)) {
      const rows = db().prepare(`
        SELECT daily_start_minute,daily_end_minute
        FROM ad_campaign_venues
        WHERE venue_id = ?
          AND campaign_id IN (
            SELECT c.campaign_id FROM ad_campaigns c WHERE c.status IN ('ACTIVE','SCHEDULED','PENDING_APPROVAL')
            OR EXISTS (SELECT 1 FROM ad_payment_requests r WHERE r.campaign_id=c.campaign_id
      ...
      `).all(venueId, adPaymentService.activationGraceMs, Date.now(), excludeCampaignId, day, day, startMinute, endMinute);
      const events = rows.flatMap(row => [[Math.max(startMinute,row.daily_start_minute),1],[Math.min(endMinute,row.daily_end_minute),-1]])
        .sort((a,b) => a[0]-b[0] || a[1]-b[1]);
      let concurrent = 0;
      if (events.some(([,delta]) => (concurrent += delta) >= MAX_CONCURRENT)) {
...
async function cleanupAdStorage() {
  ...
  const rows = db().prepare(`
    ...
    WHERE (status='EXPIRED' AND expired_at IS NOT NULL AND expired_at < ?)
       OR (status IN ('DRAFT','UPLOADING','PENDING_PAYMENT') AND created_at < ?
         AND NOT EXISTS (SELECT 1 FROM ad_payment_requests r WHERE r.campaign_id=ad_campaigns.campaign_id))
...
export function createAdRouter() {
  ...
  router.put('/settings', (req, res) => {
    // Defense in depth: server middleware must authenticate before this router.
    if (!req.admin) return res.status(401).json({ ok:false, message:'Admin sign-in required.' });
    try {
      res.json({ ok:true, splashIntervalSeconds:setAdInterval(req.body?.splashIntervalSeconds) });
  ...
  router.post('/drafts', async (req, res) => {
    try {
      ...
      if (targetVenueIds.some(venue => venue !== CURRENT_VENUE)) {
        return res.status(400).json({ ok:false, message:'Only this kiosk can receive and play your creative. Remote venues are not available yet.' });
      }
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/.test(requestedStart) || requestedStart < today ||
          !Number.isFinite(Date.parse(requestedStart)) || new Date(requestedStart).toISOString().slice(0,10) !== requestedStart) {
  ...
  router.post('/:campaignId/chunks',
    ...
    async (req, res) => {
      const uploadId = String(req.params.campaignId);
      if (uploadLocks.has(uploadId)) return res.status(409).json({ ok:false, message:'Upload is already processing. Retry shortly.' });
      ...
      } finally { uploadLocks.delete(uploadId); }
    }
  );

  router.post('/:campaignId/finalize', async (req, res) => {
    const campaignId = String(req.params.campaignId);
    if (uploadLocks.has(campaignId)) return res.status(409).json({ ok:false, message:'Upload is already processing. Retry shortly.' });
    uploadLocks.add(campaignId);
    ...
    try {
      ...
      // Claim synchronously, before filesystem awaits: concurrent finalization must not overwrite media.
      db().prepare("UPDATE ad_campaigns SET status='PROCESSING',updated_at=? WHERE campaign_id=?").run(Date.now(),campaignId);
      ownsProcessing = true;
      ...
      if (ownsProcessing) db().prepare("UPDATE ad_campaigns SET status='UPLOADING',updated_at=? WHERE campaign_id=?").run(Date.now(),campaignId);
      res.status(400).json({ ok:false, code:err.code || 'FINALIZE_FAILED', message:err.message });
    } finally { uploadLocks.delete(campaignId); }
  });
  ...
  router.post('/activate', (req, res) => {
    try {
      const result = adPaymentService.verifyCodeOnly(String(req.body?.code || ''));
      res.status(result.ok ? 200 : result.code === 'RATE_LIMITED' ? 429 : 400).json(result);
    } catch (error) { res.status(503).json({ ok:false, message:'Activation unavailable. Retry here; do not pay again.' }); }
  });
  ...
  router.get('/active-playlist', async (_req, res) => {
    ...
    res.json({ ok:true, venueId:CURRENT_VENUE, splashIntervalSeconds:getAdInterval(), ads:rows.filter(row => fs.existsSync(row.prepared_path)).map(serializeCampaign) });
  });
